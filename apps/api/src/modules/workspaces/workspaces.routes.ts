...
router.post(
  ...
  catchAsync(async (req: AuthRequest, res) => {
    ...
    if (!name || !slug) {
      throw new AppError('Workspace name and slug are required', 400, 'INVALID_PAYLOAD');
    }

    const existingWorkspace = await WorkspaceModel.findOne({ slug }).lean();
    if (existingWorkspace) {
      throw new AppError('Workspace slug already exists', 409, 'WORKSPACE_EXISTS');
    }
...
router.post(
  ...
  catchAsync(async (req: AuthRequest, res) => {
    ...
    if (!email) {
      throw new AppError('Invite email is required', 400, 'INVALID_PAYLOAD');
    }
    ...
    const requestOrigin =
      typeof req.headers.origin === 'string' ? req.headers.origin : env.CLIENT_URL;
...
router.post(
  ...
  catchAsync(async (req: AuthRequest, res) => {
    ...
    if (existingMembership) {
      ...
      const channel = workspace
        ? await ChannelModel.findOne({ workspaceId: workspace._id })
            .sort({ createdAt: 1 })
            .lean()
        : null;

      const document = workspace
        ? await DocumentModel.findOne({ workspaceId: workspace._id })
            .sort({ createdAt: 1 })
            .lean()
        : null;
    ...
    const slug = `collabspace-${String(userId).slice(-6)}`;

    const existingWorkspaceBySlug = await WorkspaceModel.findOne({ slug }).lean();
    if (existingWorkspaceBySlug) {
      const existingMember = await MemberModel.findOne({
        workspaceId: existingWorkspaceBySlug._id,
        userId,
        status: 'active'
      }).lean();
      ...
      return ok(res, {
        workspace: existingWorkspaceBySlug,
        channel,
        document,
        boardId: 'main'
      });
